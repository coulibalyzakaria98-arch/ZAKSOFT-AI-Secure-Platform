...
// On localhost → call backend directly. On any other domain (Vercel) → use relative /api/
// which is proxied to Render by vercel.json. Can still be overridden via window.ZAKSOFT_API_URL.
const _isLocal = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
const API_BASE = window.ZAKSOFT_API_URL !== undefined
  ? window.ZAKSOFT_API_URL
  : (_isLocal ? 'http://localhost:8000' : '');
